                ...
                <div class="row well">
                    <div class="col-md-12">
                        <div class="portlet light">
                            ...
                            <div class="portlet-title" style="min-height: 40px;">
                                <div class="tools"> </div>
                                <div class="btn-group">
                                    <select class="form-control select2" id="party" name="next_day" required>
                                        <option value="" selected disable>Select Party</option>
                                        @foreach($ledgers as $ledger)
                                            <option value="{{$ledger->id}}" @if($ledger->id==$data->party) selected {{$rate=$ledger->dara."/".$ledger->dara_commission."-".$ledger->akhar."/".$ledger->akhar_commission, $limit=$ledger->limit}} @endif>{{$ledger->name." ".$ledger->dara."/".$ledger->dara_commission}}</option>
                                        @endforeach
                                    </select>                                
                                </div>   

                                <div class="btn-group">
                                    <a  href="javascript:;" id="rate" class="btn green"> Rate: @if($rate) {{$rate}} @else 0/0-0/0 @endif</a>
                                </div>
                                <div class="btn-group">
                                    <a  href="javascript:;" id="limit" class="btn btn-success"> Limit: @if($limit) {{$limit}} @else 0 @endif </a>
                                </div>
                                <div class="btn-group">
                                    <a  href="javascript:;" id="capping" class="btn red"> Capping: 0 </a>
                                </div>
                                <div class="btn-group fRight" style="margin-left: 5px;">
                                    <a  href="javascript:;" class="btn red"> Time Left: <span id="hour_left">00</span>:<span id="minute_left">00</span>:<span id="second_left">00</span> </a>
                                </div>
                                <div class="btn-group fRight">
                                    <a  href="javascript:;" id="date" class="btn red"> Date: {{date('d-m-Y',strtotime($shift->date))}} </a>
                                </div>
                            </div>
                            
                            <div class="portlet-body" style="padding-top: 0px;">
                                <div class="row">
                                    <div class="col-md-3 normal_view">
                                        <div class="row form-group">
                                            <div class="col-md-5" style="padding-left:5px;padding-right:5px;">
                                                <input type="text" onkeyup onkeypress="return isNumberKey(event)" maxlength="4" placeholder="Number" class="form-control number number1"/> 
                                            </div>
                                            <div class="col-md-5" style="padding-left:5px;padding-right:5px;">
                                                <input type="text" onkeyup onkeypress="return isNumberKey(event)" maxlength="4" placeholder="Amount" class="form-control amount amount1"/> 
                                            </div>
                                            <div class="col-md-1" style="padding-left:5px;padding-right:5px;">
                                                <button type="button" class="btn green" onclick="transaction_fields();">+</button> 
                                            </div>
                                        </div>
                                        <div id="transaction_fields" style="height:400px; overflow-y: scroll;">
                                            @php $numbers=json_decode($data->number); $amounts=json_decode($data->amount); @endphp
                                            @for($i=0;$i<sizeof($amounts);$i++)
                                                <div class="row form-group removeclass{{$i}}">
                                                    <div class="col-md-5" style="padding-left:5px;padding-right:5px;">
                                                        <input type="number" min="0" name="number[]" placeholder="Number" disabled class="form-control number" value="{{$numbers[$i]}}" />
                                                    </div>
                                                    <div class="col-md-5" style="padding-left:5px;padding-right:5px;">
                                                        <input type="number" min="0" name="amount[]" placeholder="Amount" disabled class="form-control amount" value="{{$amounts[$i]}}" /> 
                                                    </div>
                                                    <div class="col-md-2" style="padding-left:5px;padding-right:5px;">
                                                        <button type="button" class="btn red" onclick="remove_transaction_fields({{$i}});">X</button> 
                                                    </div>
                                                </div>
                                            @endfor
                                        </div>
                                    </div>
                                    <div class="col-md-6 normal_view">
                                        <ul class="list-group">
                                            <li class="list-group-item bg-blue bg-font-blue"> Utar Mode Instructions </li>
                                            <li class="list-group-item"> 1. Dara Number: should be 1 to 100 </li>
                                            <li class="list-group-item"> 2. Bahar Akhar Number: should be 000 to 999</li>
                                            <li class="list-group-item"> 3. Andar Akhar Number: should be 0000 to 9999 </li>
                                            <li class="list-group-item"> 4. Press F12 for Jantri View </li>
                                            <li class="list-group-item"> 5. Press `~ for Re-Focus </li>
                                            <li class="list-group-item sbold" style="text-align: center;"> Total Count : <span id="amount_count">0</span> </li>
                                        </ul>
                                    </div>
                                    <div class="col-md-9 jantri_view" style="display:none;">
                                    ...
                                    <div class="col-md-3">
                                        <ul class="list-group">
                                            <li class="list-group-item bg-green bg-font-green sbold" style="text-align: center;"><span class="blink_text"> {{$shift->name}} [ Live ] </span></li>
                                        </ul>
                                        <table style="margin-bottom: 0px; width:100%" class="table table-bordered">
                                            <thead>
                                                <tr class="bg-blue bg-font-blue">
                                                    <th style="padding:7px!important; text-align:center;" colspan="2">Applied Narration</th>
                                                </tr>
                                            </thead>
                                            <tbody id="AppliedNarrationPanel">
                                            </tbody>
                                        </table>
                                        <ul class="list-group">
                                            <li class="list-group-item bg-red sbold" style="text-align: center;color:white;">Grand Total: <span id="GrandTotal">0</span></li>
                                        </ul>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="portlet-title"> 
                                <div class="btn-group fRight" style="margin-left: 5px;">
                                    <a  href="javascript:;" id="clear" class="btn yellow"> Clear </a>
                                </div>
                                <div class="btn-group fRight" style="margin-left: 5px;">
                                    <a  href="javascript:;" id="save" class="btn green"> Save Now (F2) </a>
                                    <button style="display:none;" id="wait" class="btn yellow"><i class="icon-spinner"></i>Please Wait...</button>    
                                </div>
                                <div class="btn-group fRight" style="margin-left: 5px;">
                                    <a  href="javascript:;" id="random_f8" class="btn btn-success"> Random (F8) </a>
                                </div>
                                <div class="btn-group fRight" style="margin-left: 5px;">
                                    <a  href="javascript:;" id="random_f7" class="btn btn-success"> From-To (F7) </a>
                                </div>
                                <div class="btn-group fRight" style="margin-left: 5px;">
                                    <a  href="javascript:;" id="random_f6" class="btn btn-success"> Cross (F6) </a>
                                </div>
                                <div class="btn-group fRight" style="margin-left: 5px;">
                                    <a  href="javascript:;" id="random_f4" class="btn btn-success"> Random (F4) </a>
                                </div>
...
<!-- Add random_f4 modal -->
<div class="modal fade bs-modal-sm" id="random_f4_model" tabindex="-1" role="dialog" aria-hidden="true">
    <div class="modal-dialog modal-sm">
        <div class="modal-content">
            <div class="modal-header bg-blue bg-font-blue">
                <button type="button" class="close" data-dismiss="modal" aria-hidden="true" onclick=clear_table("random_table")></button>
                <h4 class="modal-title" id="model_title">Random</h4>
            </div>
                <div class="modal-body">  
                    <div class="portlet-body">
                        <div class="row">
                            <table id="random_table" style="width:100%;">
                                <tbody id="RandomTableOld">
                                    <tr>
                                        <td style="width:30px;"></td>
                                        <td> <b>NUMBER</b> </td>
                                        <td>
                                            <input type="text" onkeyup="setRandomOld_TotalAmount()" onkeypress="return isNumberKey(event)" style="text-align:right; width:100px; margin: 5px;" value="" maxlength="4" name="RandomNumberOld[]" id="REN_Number_input" onkeydown="Random_clone()" class="form-control">
                                        </td>
                                    </tr>
                                    <tr>
                                        <td style="width:30px;"></td>
                                ...
                                <tfoot>
                                    <tr>
                                        ...
                                        <th> <b>AMOUNT</b> </th>
                                        <th>
                                            <input type="text" onkeyup="setRandomOld_TotalAmount()" onkeypress="return isNumberKey(event)" style="text-align:right; width:100px; margin: 5px;" value="" id="RandomAmountOld" name="RandomAmountOld" class="form-control" placeholder="Amount">
                                        </th>
                                    </tr>
                                    <tr>
                                        <th></th>
                                        <th> <b>PLT-AMOUNT</b> </th>
                                        <th>
                                            <input type="text" onkeyup="setRandomOld_TotalAmount()" onkeypress="return isNumberKey(event)" style="text-align:right; width:100px; margin: 5px;" value="" id="RandomAmountOldPlt" name="RandomAmountOldPlt" class="form-control" placeholder="Plt-Amount">
                                        </th>
                                    </tr>
                                </tfoot>
                            </table>
                            <div class="col-12 text-center"><b>TOTAL AMOUNT : <span id="OldRandPanelTotalAmount">0</span></b></div>
                        </div>
                    </div>
                </div>
                ...
                <div class="modal-footer">               
                    ...
                    <button id="save_f4" class="btn green" onclick="javascript:setRandomOld();">Save</button> 
                </div>
        </div>
        <!-- /.modal-content -->
    </div>
...
<!-- Add random_f6 modal -->
<div class="modal fade bs-modal-md" id="random_f6_model" tabindex="-1" role="dialog" aria-hidden="true">
    <div class="modal-dialog modal-md">
        <div class="modal-content">
            <div class="modal-header bg-blue bg-font-blue">
                <button type="button" class="close" data-dismiss="modal" aria-hidden="true" onclick=clear_table("random_f6")></button>
                <h4 class="modal-title" id="model_title">Cross</h4>
            </div>
            <div class="modal-body">  
                <div class="portlet-body">
                    <table class="table" id="random_f6" style="width:100%;" border="2" cellspacing="3px">
                        <thead>
                            <tr class=" bg-blue bg-font-blue">
                                <th style="text-align:center; vertical-align: middle;">ANDER</th>
                                <th style="text-align:center;  vertical-align: middle;">BAHAR</th>
                                <th style="text-align:center; vertical-align: middle;">AMOUNT</th>
                                <th style="text-align:center; vertical-align: middle;">JODA</th>
                            </tr>
                            <tr>
                                <th style="padding:2px !important; margin:0px auto; width:120px;">
                                    <input type="text" onkeyup="setCrossingCount()" onkeypress="return isNumberKey(event)" style="text-align:center;" maxlength="10" name="Cross_Number1" id="Cross_Number1" class="form-control" placeholder="NUMBER 1">
                                </th>
                                <th style="padding:2px !important; margin:0px auto; width:120px;">
                                    <input type="text" onkeyup="setCrossingCount()" onkeypress="return isNumberKey(event)" style="text-align:center;" maxlength="10" name="Cross_Number2" id="Cross_Number2" class="form-control" placeholder="NUMBER 2">
                                </th>
                                <th style="padding:2px !important; margin:0px auto;">
                                    <input type="text" onkeypress="return isNumberKey(event)" style="text-align:center;" maxlength="5" name="Cross_Amount" id="Cross_Amount" class="form-control" placeholder="AMT">
                                </th>
                                <th style="padding:2px !important; margin:0px auto; width:60px;">
                                    <input onkeyup="setCrossingCount()" type="text" style="text-align:center;" maxlength="1" name="Cross_Joda" id="Cross_Joda" class="form-control" value="Y" placeholder="JODA">
                                </th>
                            </tr>
                        </thead>
                    </table>
                    <div class="col-12"><h4>TOTAL CROSS COUNT : <span id="CrossModalNumberCount">0</span></h4></div>
                    <div class="col-12"><h4>TOTAL AMOUNT : <span id="CrossModalTotalAmount">0</span></h4></div>
                    <div class="col-12">1. अगर JODA "Y" है तो 00-99 JODA की ENTRY होगी</div>
                    <div class="col-12">2. अगर BOTH NUMBER BLANK है और JODA "Y" है तो सिर्फ 00-99 JODA की ENTRY होगी</div>
                </div>
            </div>

            <div class="modal-footer">               
                {{-- <button type="button" class="btn dark btn-outline" data-dismiss="modal">Close</button> --}}
                <button class="btn green" onclick="setCrossing()">Save</button> 
            </div>
        </div>
    </div>
</div>
...
<!-- Add random_f8 modal -->
<div class="modal fade bs-modal-md" id="RandomModal" tabindex="-1" role="dialog" aria-hidden="true">
    <div class="modal-dialog modal-md">
        <div class="modal-content">
            <div class="modal-header bg-blue bg-font-blue">
                <button type="button" class="close" data-dismiss="modal" aria-hidden="true" onclick=clear_table("random_f8")></button>
                <h4 class="modal-title" id="model_title">Random</h4>
            </div>
            <div class="modal-body">  
                <div class="portlet-body random_f8">
                    <div class="row gutters">
                        <div class="col-xl-12 col-lglg-12 col-md-12 col-sm-12 col-12">
                            <div class="form-group">
                                <label for="inputReadOnly">NOTE: Dara number should be 2 digit without any separator.</label>
                                <label for="inputReadOnly">NOTE: Akhar number should be 1 digit without any separator.</label>
                            </div>
                        </div>
                    </div>
                    <div class="row gutters">
                        <div class="col-xl-8 col-lglg-8 col-md-8 col-sm-8 col-12">
                            <div class="form-group">
                                <label for="inputReadOnly">Dara</label>
                                <textarea type="text" onkeyup="" onkeypress="return isNumberKey(event)" style=" height:90px;" name="Random_Number" id="Random_Number" class="form-control" placeholder="Numbers, eg: 01 and 89 like: 0189"></textarea>
                            </div>
                        </div>
                        <div class="col-xl-4 col-lglg-4 col-md-4 col-sm-4 col-12">
                            <div class="form-group">
                                <label class="d-block">Amount <b class="float-right" style="color:#2d9b5a;" id="Total_Random_Dara_Amount"></b></label>
                                <input type="text" onkeyup="" onkeypress="return isNumberKey(event)" style="text-align:center; width:150px;" name="Random_Amount" id="Random_Amount" maxlength="7" class="form-control" placeholder="Amount">
                            </div>
                        </div>
                    </div>
                    <div class="row gutters">
                        <div class="col-xl-8 col-lglg-8 col-md-8 col-sm-8 col-12">
                            <div class="form-group">
                                <label for="inputReadOnly">Akhar Bahar</label>
                                <input type="text" onkeyup="" onkeypress="return isNumberKey(event)" style="" name="RandomBahar_Number" id="RandomBahar_Number" class="form-control" placeholder="Numbers, eg: 1 and 2 like: 12">
                            </div>
                        </div>
                        <div class="col-xl-4 col-lglg-4 col-md-4 col-sm-4 col-12">
                            <div class="form-group">
                                <label class="d-block">Amount <b class="float-right" style="color:#2d9b5a;" id="Total_Random_Bahar_Amount"></b></label>
                                <input type="text" onkeyup="" onkeypress="return isNumberKey(event)" style="text-align:center; width:150px;" name="RandomBahar_Amount" id="RandomBahar_Amount" maxlength="7" class="form-control" placeholder="Amount">
                            </div>
                        </div>
                    </div>
                    <div class="row gutters">
                        <div class="col-xl-8 col-lglg-8 col-md-8 col-sm-8 col-12">
                            <div class="form-group">
                                <label for="inputReadOnly">Akhar Andar</label>
                                <input type="text" onkeyup="" onkeypress="return isNumberKey(event)" style="" name="RandomAndar_Number" id="RandomAndar_Number" class="form-control" placeholder="Numbers, eg: 1 and 2 like: 12">
                            </div>
                        </div>
                        <div class="col-xl-4 col-lglg-4 col-md-4 col-sm-4 col-12">
                            <div class="form-group">
                                <label class="d-block">Amount <b class="float-right" style="color:#2d9b5a;" id="Total_Random_Andar_Amount"></b></label>
                                <input type="text" onkeyup="" onkeypress="return isNumberKey(event)" style="text-align:center; width:150px;" name="RandomAndar_Amount" id="RandomAndar_Amount" maxlength="7" class="form-control" placeholder="Amount">
                            </div>
                        </div>
                    </div>
                    <div class="col-12 text-center"><b>TOTAL AMOUNT : <span id="Total_Random_Amount">0</span></b></div>
                </div>
            </div>
            <div class="modal-footer">               
                {{-- <button type="button" class="btn dark btn-outline" data-dismiss="modal">Close</button> --}}
                <button type="button" onclick="setRandom()" class="btn green">Save</button> 
            </div>
        </div>
    </div>
</div>
...
<!-- Add random_f7 modal -->
<div class="modal fade bs-modal-md" id="FromToModal" tabindex="-1" role="dialog" aria-hidden="true">
    <div class="modal-dialog modal-md">
        <div class="modal-content">
            <div class="modal-header bg-blue bg-font-blue">
                <button type="button" class="close" data-dismiss="modal" aria-hidden="true" onclick=clear_table("random_f7")></button>
                <h4 class="modal-title" id="model_title">From-To</h4>
            </div>
            <div class="modal-body">  
                <div class="portlet-body">
                    <table class="table" id="random_f7" style="width:100%;" border="2" cellspacing="3px">
                        <thead>
                        <tr class="bg-blue bg-font-blue">
                        ...
                        <tr>
                            <th style="padding:2px !important; margin:0px auto; width:120px;">
                            <input type="text" onkeyup="setFromTo_TotalAmount()" onkeypress="return isNumberKey(event)" style="text-align:center;" maxlength="2" name="From_Number" id="From_Number" class="form-control" placeholder="From">
                            </th>
                            <th style="padding:2px !important; margin:0px auto; width:120px;">
                            <input type="text" onkeyup="setFromTo_TotalAmount()" onkeypress="return isNumberKey(event)" style="text-align:center;" maxlength="3" name="To_Number" id="To_Number" class="form-control" placeholder="To">
                            </th>
                            <th style="padding:2px !important; margin:0px auto;">
                            <input type="text" onkeyup="setFromTo_TotalAmount()" onkeypress="return isNumberKey(event)" style="text-align:center;" maxlength="5" name="FromTo_Amount" id="FromTo_Amount" class="form-control" placeholder="Amount">
                            </th>
                        </tr>
                        <tr>
                            ...
                            <th style="padding:2px !important; margin:0px auto;">
                            <input type="text" onkeyup="setFromTo_TotalAmount()" onkeypress="return isNumberKey(event)" style="text-align:center;" maxlength="5" name="FromTo_AmountPlt" id="FromTo_AmountPlt" class="form-control" placeholder="Plt-Amount">
                            </th>
                        </tr>
                        </thead>
                    </table>
                    <div class="row">
                        <div class="col-md-12 sbold"><h4><b>TOTAL AMOUNT : <span id="FromToModalTotalAmount">0</span><b></h4></div>
                    </row>
            ...
            <div class="modal-footer">               
                {{-- <button type="button" class="btn dark btn-outline" data-dismiss="modal">Close</button> --}}
                <button type="button" onclick="setFromTo()" id="save_f7" class="btn green">Save</button> 
            </div>
        </div>
    </div>
</div>
...
<input type="hidden" id="view_type" value="normal" />

@endsection
@section('js')
<script>
    var room = {{sizeof($amounts)}};
</script>
<script src={{URL::asset("/js/knockout-2.2.1.js")}}  type="text/javascript"></script>
<script src={{URL::asset("/js/transaction_add.js")}}  type="text/javascript"></script>
<script>
    ...
    function makeTimer() {
        ...
        $("#hour_left").html(hours);
        $("#minute_left").html(minutes);
        $("#second_left").html(seconds);		
    ...
    $("#save").click(function(){
        ...
        if ($('#party').val()=="") {
            toastr.error("Select any party first !!", 'Error');
            return false;
        }
        ...
        if (amounts.length<1) {
            toastr.error("Enter atleast one amount and number to add transaction!!", 'Error');
            return false;
        }

        if (!$('#total_amount').val()) {
            toastr.error("Amount or number can not be null !!", 'Error');
            return false;
        }

        $.ajax({		            	
            type: "POST",
            url: `${window.pageData.baseUrl}/api/update_transaction`,
            data: {id:{{$data->id}},total:$("#total_amount").val(),amount:amounts,number:numbers,party:$('#party').val(),user:{{Auth::User()->id}},DaraTotal:$('#JantriDaraTotal').text(),AkharTotal:parseFloat($('#JantriBaharTotal').text())+parseFloat($('#JantriAndarTotal').text())},                                     
            beforeSend: function(){
                $("#save").hide(); 
                $("#wait").show();
            },
